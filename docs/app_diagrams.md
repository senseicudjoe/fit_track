...
## 8. Demo Walkthrough — A Day in the Life of Marcus

This section narrates a single end-to-end demo that exercises every
major capability the diagrams describe. The protagonist, **Marcus**, is
a 28-year-old software engineer who has just decided to get serious
about consistency at the gym. Each scene of the demo names the route the
user is on, calls out the API or service exercised behind the scenes,
and notes the data that ends up persisted as a result. The intent is to
give a presenter a complete script for a live walkthrough.

### Scene 1 — First Launch and Sign-Up

Marcus installs FitTrack from the store and taps the icon. The
`/` splash screen appears for a fraction of a second while
`Firebase.initializeApp` resolves and `NotificationService.init` loads
the timezone database. With no existing session the router redirects to
`/intro`, where Marcus reads the value proposition and taps
**Create Account**, landing on `/register`.

He enters his email, a password, and "Marcus" as his display name and
taps **Sign Up**. Behind the scenes `AuthProvider.register` calls
`AuthService.register`, which invokes
`FirebaseAuth.createUserWithEmailAndPassword`, sets the display name on
the new credential, and writes a starter document to `users/{uid}` in
Firestore with `isOnboarded: false`. The router observes the auth-state
change through `refreshListenable`, sees Marcus is logged in but not
onboarded, and forwards him to `/onboarding`.

*APIs exercised:* Firebase Auth, Cloud Firestore (`users` collection).

### Scene 2 — Onboarding

The onboarding screen asks Marcus for his age (28), weight (78 kg),
height (180 cm), primary fitness goal ("Build strength"), and activity
level ("Intermediate"). When he taps **Continue**, the screen calls
`AuthService.updateUser` to merge the new fields into his profile
document and flips `isOnboarded` to true. The router immediately
forwards him to `/permissions`.

On the permissions screen Marcus grants notification access (so
reminders can fire) and activity-recognition access (so the pedometer
can read his step count). `permission_handler` brokers the OS dialogues
and `NotificationService.requestPermissions` registers the app for
alerts on both Android and iOS. With both grants in hand, the router
finally forwards him to `/dashboard`.

*APIs exercised:* Cloud Firestore (`users` update),
`permission_handler`, `flutter_local_notifications` permission request.

### Scene 3 — First Look at the Dashboard

Marcus arrives on `/dashboard`. Three things happen in parallel.
`StatsProvider` calls `FirestoreService.fetchTodayStats`, which lazily
creates an empty `dailyStats/{today}` document if none exists.
`PedometerService.start` subscribes to `Pedometer.stepCountStream` and
seeds today's count from the SQLite cache. The dashboard widget tree
binds to those providers, so the **Steps** card shows zero initially
and begins ticking upward as soon as Marcus moves.

Marcus sees four cards: today's steps (0), calories burned (0),
workout count (0), and a goal progress widget that is empty for now.
The bottom navigation bar shows **Home**, **Log**, **Goals**,
**Progress**, and **Profile**.

*APIs exercised:* Cloud Firestore (`dailyStats`), pedometer plugin,
SQLite step cache.

### Scene 4 — Setting His First Goal

Marcus taps the **Goals** tab. The empty state invites him to create his
first goal. He fills in "Run 20 km this week" — type "Distance", target
20, unit "km", deadline next Sunday — and taps **Save**.
`GoalProvider.addGoal` builds a `GoalModel` with a fresh UUID and calls
`FirestoreService.saveGoal`, which writes to
`users/{uid}/goals/{goalId}`. The new goal pops into the list
immediately because the screen subscribes to `goalsStream`, which is
backed by Firestore's snapshot listener.

*APIs exercised:* Cloud Firestore (`goals` sub-collection), `uuid`.

### Scene 5 — Scheduling a Reminder

To stay consistent, Marcus jumps to **Profile → Reminders** and creates
a new reminder titled "Evening run" at 18:30, repeating Monday through
Friday. Tapping **Save** triggers `FirestoreService.saveReminder` to
persist the reminder document and `NotificationService.scheduleReminder`
to enqueue five recurring local notifications — one per weekday —
through `flutter_local_notifications.zonedSchedule`. Because the
notification channel is `workout_reminders` with high importance, the
alerts will pierce Do Not Disturb on Android.

*APIs exercised:* Cloud Firestore (`reminders`),
`flutter_local_notifications`, `flutter_timezone`, OS alarm manager.

### Scene 6 — Logging a HIIT Workout with the Timer

The next morning Marcus opens the app, taps the **Log** tab, and
selects **HIIT** as the workout type. He taps **Use Timer**, which
pushes him onto `/timer` (a full-screen route, no bottom bar). On the
timer screen he configures eight rounds of forty seconds work and
twenty seconds rest, then taps **Start**.

`TimerProvider` delegates to `TimerService`, which starts a
`Timer.periodic` ticker, transitions through `work → rest` cycles, and
records per-round splits. Marcus follows along with burpees and
mountain climbers. When the cooldown completes, the service emits
`onComplete` with the captured `List<int>` of splits. The timer screen
returns to `/log` carrying that list as a result.

Back on `/log`, Marcus enters a duration of twelve minutes, an
estimated calorie burn of 180, leaves distance blank, and taps
**Save Workout**. `WorkoutProvider.addWorkout` calls
`FirestoreService.saveWorkout` to persist a `WorkoutModel` with
`type: "HIIT"`, the captured `timerSplits`, and `loggedAt = now`. The
provider then asks `StatsProvider.refreshDailyStats` to upsert today's
aggregate via `FirestoreService.updateTodayStats`. A "Workout saved"
toast confirms success.

*APIs exercised:* `dart:async` ticker, Cloud Firestore (`workouts` and
`dailyStats`).

### Scene 7 — Reviewing Progress

Marcus taps the **Progress** tab. The screen subscribes to
`workoutsStream`, which streams the most recent twenty entries ordered
by `loggedAt`. His HIIT entry sits at the top with the timer splits
visualised as a bar strip. He taps the card, which pushes him to
`/workout/:id` — `WorkoutDetailScreen` — where he can read the notes,
review each round split, or delete the entry via
`FirestoreService.deleteWorkout`.

Curious about the bigger picture, he taps the **Insights** tab. The
insights screen calls `FirestoreService.fetchStatRange` for the last
seven days and renders the result in `fl_chart` line and bar charts:
calories per day, workouts per day, and a step-count area chart. The
goal-progress card on the same screen shows that he is at 0 of 20 km
this week — accurate, because today's HIIT did not contribute to the
"Distance" goal.

*APIs exercised:* Cloud Firestore (`workouts` stream and `dailyStats`
range query), `fl_chart`.

### Scene 8 — Editing the Profile and Signing Out

At the end of the demo Marcus taps **Profile**, then **Edit profile**,
which routes him to `/edit-profile` (full-screen, no bottom bar). He
updates his weight to 77.5 kg and saves; `AuthService.updateUser`
merges the change into `users/{uid}` so future calorie calculations
reflect the new value. He returns to settings and taps **Sign Out**.
`AuthProvider.signOut` calls `AuthService.signOut`, which signs out of
both Firebase Auth and Google. The router's `refreshListenable` fires,
the redirect rules see `isLoggedIn` is now false, and Marcus lands back
on the splash screen — ready for the next demo run.

*APIs exercised:* Cloud Firestore (`users` update), Firebase Auth
sign-out, Google Sign-In sign-out.

### 8.1 Demo Cheat Sheet

The table below collapses the eight scenes into a one-line summary that
a presenter can keep on a second screen.

| # | Scene                       | Route(s)                       | Headline API call                                |
|---|-----------------------------|--------------------------------|--------------------------------------------------|
| 1 | First launch and sign-up    | `/` → `/intro` → `/register`   | `FirebaseAuth.createUserWithEmailAndPassword`    |
| 2 | Onboarding and permissions  | `/onboarding` → `/permissions` | `AuthService.updateUser`, `permission_handler`   |
| 3 | First dashboard             | `/dashboard`                   | `Pedometer.stepCountStream`, `fetchTodayStats`   |
| 4 | Create a goal               | `/goals`                       | `FirestoreService.saveGoal`                      |
| 5 | Schedule a reminder         | `/reminders`                   | `flutter_local_notifications.zonedSchedule`      |
| 6 | Log a HIIT workout          | `/log` ↔ `/timer`              | `TimerService` ticker, `FirestoreService.saveWorkout` |
| 7 | Review progress / insights  | `/progress`, `/insights`       | `workoutsStream`, `fetchStatRange`               |
| 8 | Edit profile and sign out   | `/edit-profile`, `/settings`   | `AuthService.updateUser`, `AuthService.signOut`  |
